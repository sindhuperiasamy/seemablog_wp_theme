...
(function($) {// start closure
	...
	$(function(){
		...
		// Create containers
			$('body')
				.append('<div id="overlay01"></div><div id="jb-window"><div id="jb-window-inner"><div id="jb-window-content" style="width: auto; height: auto;" tabindex="0"></div></div></div>')
	...
	openLightBox = function (e) {
		...
		// if lightbox is open already and fragment was reused... clean up
			if ($('#overlay01').is(':visible')) {
					options = $('#overlay01').data('options');
		...
		// grab previousOptions
			previousOptions = $('#overlay01').data('options') || {};
		
			
		// assign link options to lightbox
			$(this).data('options', options);
			$('#overlay01').data('options', options);
		...
		// if lightbox is closed:
			...
			// show overlay
				$('#overlay01').fadeIn(options.animationSpeed).centre();
	...
	closeLightBox = function (evt) {
		...
		$('#overlay01').fadeOut(options.animationSpeed);
		$('#jb-window').hide();
	...
	lightBoxKeypress = function (evt) {
		...
		// handle keypresses here
			switch (evtKeyCode) {
				...
				break;
				case DOM_VK_HOME:
					$($('#overlay01').data('options').gallerySelector).firstInSet().click();
				break;
				case DOM_VK_END:
					$($('#overlay01').data('options').gallerySelector).lastInSet().click();
				break;
				case DOM_VK_ESCAPE:
					closeLightBox.apply();
				break;
